...
public class Moving  extends JPanel {
    ...
    private int width;
    private int height;
    private int moveX = 45;
    private  int moveY = 45;
    ...
    private void drawGrid(Graphics g) {
        ...
        for(int x=width/2 - moveX; x<width; x+=30){  // цикл от центра до правого края
            g.drawLine(x, 0, x, height);    // вертикальная линия
        }

        for(int x=width/2 - moveX; x>0; x-=30){  // цикл от центра до леваого края
            g.drawLine(x, 0, x, height);   // вертикальная линия
        }

        for(int y=height/2 + moveY; y<height; y+=30){  // цикл от центра до верхнего края
            g.drawLine(0, y, width, y);    // горизонтальная линия
        }

        for(int y=height/2 + moveY; y>0; y-=30){  // цикл от центра до леваого края
            g.drawLine(0, y, width, y);    // горизонтальная линия
        }
    }
    ...
    private void drawAxis(Graphics g) {
        ...
        g.drawLine(0, height/2 + moveY, width, height/2 + moveY);
    }
